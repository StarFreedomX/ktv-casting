...
use crate::dlna_controller::{DlnaController, generate_didl_metadata, get_local_ip};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 测试媒体文件
    let test_video = "test_videos/test1.mp4";
    ...
    // 设置AVTransport URI
    println!("设置媒体URI...");
    if let Err(e) = controller.set_avtransport_uri(
        device,
        &format!("/{}", test_video),
        &metadata,
        local_ip,
